...
func QueryOracleDBSQLMatchText(text string) error {
	columns, values, err := db.Query(fmt.Sprintf(`select sql_id,
       ...
       to_char(hash_value) hash_value,
       to_char(plan_hash_value) plan_hash_value,
       to_char(child_number) child_number,
       sql_text
  from v$sql
 where upper(sql_text) like '%%%s%%'
   and sql_text not like '%%v$sql%%'`, strings.ToUpper(text)))
	if err != nil {
		return err
	}
	util.NewMarkdownTableStyle(os.Stdout, columns, values)
...
func QueryOracleDBSQLSummaryBySQLID(sqlID, startTime, endTime string) error {
 ...
 MODULE,
 MACHINE,
 to_char(SQL_PLAN_HASH_VALUE) SQL_PLAN_HASH_VALUE,
...
func QueryOracleDBSQLPlanByAWR(sqlid, planhashvalue string) error {
  ...
  FROM table(dbms_xplan.display_awr('%s',
	...
	util.NewColMarkdownTable(os.Stdout, columns, values)
	return nil
}
...
func QueryOracleDBSQLPlanByXplan(sqlid, childnumber, format string) error {
	...
	if err == sql.ErrNoRows {
		fmt.Println("package xplan not existed, Would auto make package xplan")
...
func makeOracleDBXplanPackage() error {
	sql1 := `CREATE OR REPLACE TYPE xplan_ot force AS OBJECT( plan_table_output VARCHAR2(300) );`
...
END xplan;`
	...
	sql4 := `CREATE OR REPLACE PACKAGE BODY xplan AS

  TYPE ntt_order_map_binds IS TABLE OF VARCHAR2(100);

  TYPE aat_order_map IS TABLE OF PLS_INTEGER INDEX BY PLS_INTEGER;
  ...
  PROCEDURE build_order_map(p_sql   IN VARCHAR2,
    ...
    TYPE aat_id_data IS TABLE OF rt_id_data INDEX BY PLS_INTEGER;
  ...
  BEGIN
    ...
    -- Build SQL template...
    -- ---------------------
    v_sql := 'WITH sql_plan_data AS ( ' || p_sql || '
                        )
    ...
    CASE p_binds.COUNT
      ...
      WHEN 2 THEN
        OPEN v_cursor FOR v_sql
          USING p_binds(1), TO_NUMBER(p_binds(2));
      WHEN 3 THEN
        OPEN v_cursor FOR v_sql
          USING p_binds(1), TO_NUMBER(p_binds(2)), TO_NUMBER(p_binds(3));
    END CASE;
  ...
  ----------------------------------------------------------------------------
  FUNCTION prepare_row(p_curr IN VARCHAR2, p_next IN VARCHAR2)
    RETURN xplan_ot IS
...
END xplan;`
	...
	currentUser := db.ORA.QueryRow(`select user from dual`)
	sql5 := fmt.Sprintf(`GRANT SELECT ANY DICTIONARY TO %s`, currentUser)
